  ...
  return (
    <motion.header variants={headerVariant} animate={hidden && !toggleIcon ? "hidden" : "visible"} className={`z-[9999999] ${bg ? profile || blue ? "bg-[#3D217A] dark:bg-black" : "bg-white text-black" : profile || blue ? "bg-[#3D217A] dark:bg-black" : "bg-transparent"} fixed top-0 right-0 left-0 w-full flex flex-row justify-between items-center px-4 lg:px-10 py-3 lg:py-0 duration-300 text-black`}>
      ...
      <div onClick={handleToggle} className="z-[9999999999999] exl:hidden cursor-pointer ">
        {toggleIcon ? (
          <XMarkIcon width={35} className={`${bg || !profile ? "text-black dark:text-white" : "text-white dark:text-black"} dark:text-black`} />
        ) : (
          <Bars3BottomRightIcon width={35} className={`${bg || blue || profile ? "text-black dark:text-white" : "text-white dark:text-black"} dark:text-black`} />
        )}
      </div>
      <AnimatePresence>
