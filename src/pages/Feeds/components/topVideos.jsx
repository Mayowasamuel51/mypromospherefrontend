            ...
            {data?.data?.videos.map((video) => (
                <div key={video.id} className='flex flex-col gap-4'>
                    <div className='w-full aspect-ratio-box rounded-lg overflow-hidden'>
                        <ReactPlayer width={280} height={300} style={{borderRadius: "8px"}} url={video?.titlevideourl} playing={true} light={video.id % 2 === 0 ? thumbnail1 : thumbnail2} loop={true} muted={true} playIcon={<FaRegCirclePlay size={50} color='#fff' />} className="w-fit hover:outline hover:outline-2 hover:outline-[#3D217A]" />
                    </div>
