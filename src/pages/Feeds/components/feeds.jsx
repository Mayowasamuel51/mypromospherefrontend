              ...
              <Splide options={{
                ...
                <SplideSlide className='rounded-md w-fit'>
                  <Link to={`/feed/${item.id}`}>
                    <LazyLoadImage width={`100%`} visibleByDefault={item.src === item.itemadsimagesurls} effect='blur' src={`https://apimypromospheretest.com.ng/public/storage/${item.titleImageurl.slice(7)}`} alt="" style={{ width: 280, height: 300 }} className="w-full rounded-md object-cover" />
                  </Link>
                </SplideSlide>
                {data?.data?.other_images.filter((img) => img.itemfree_ads_id === item.id).map((img, index, arr) => arr.length > 0 && (
                  <SplideSlide key={img.id} className='rounded-md w-fit'>
                    <Link to={`/feed/${item.id}`}>
                      <LazyLoadImage width={`100%`} visibleByDefault={img.src === img.itemadsimagesurls} effect='blur' src={img.itemadsimagesurls} alt="" style={{ width: 280, height: 300 }} className="w-full rounded-md object-cover" />
                    </Link>
                  </SplideSlide>
                ))}
              </Splide> :
