    ...
    return (
        <motion.section variants={containerVariant} initial="initial" animate="animate" exit="exit" className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-80">
            <motion.div variants={divVariant} className="relative flex flex-col gap-6 w-[80%] md:w-[750px] bg-white dark:bg-darkBg py-4 px-4 md:px-6 rounded-md">
                <FaXmark size={40} className="z-[99999999999] text-white absolute -top-10 -right-10" onClick={() => setComment(false)} />
                {!token && <input className="w-full md:h-14 h-12 bg-slate-100 dark:bg-inputDark border rounded-md" type="text" />}
                <textarea name="" id="" className="md:p-4 p-1 w-full resize-none md:h-44 bg-slate-100 dark:bg-inputDark border rounded-md "></textarea>
